...
<head>
	...
	<script data-relocate="true">
		...
		$(function() {
			...
			$("[data-more-awards]").each(function() {
				new UNKSO.Ajax.Popup($(this));
			});
		});
		//]]>
	</script>
...
{capture assign='__pageTitle'}
	{$conversation->subject} {if $conversation->isClosed} <i class="icon icon32 icon-lock jsTooltip jsIconLock" style="vertical-align:0;" title="{lang}wcf.global.state.closed{/lang}"></i>{/if}
{/capture}
{include file='header' title=$__pageTitle light=true}
...
<div class="container">
  ...
  <div class="contentNavigation">
  	...
  	<nav>
  		<ul class="conversation jsConversationInlineEditorContainer" data-conversation-id="{@$conversation->conversationID}" data-label-ids="[ {implode from=$conversation->getAssignedLabels() item=label}{@$label->labelID}{/implode} ]" data-is-closed="{@$conversation->isClosed}" data-can-close-conversation="{if $conversation->userID == $__wcf->getUser()->userID}1{else}0{/if}" data-can-add-participants="{if $conversation->canAddParticipants()}1{else}0{/if}">
  			<li class="jsOnly"><a href="#" class="btn btn-secondary btn-3d jsConversationInlineEditor"><span class="icon icon16 icon-pencil"></span> <span>{lang}wcf.global.button.edit{/lang}</span></a></li>
  			{if !$conversation->isClosed}<li><a href="{link controller='ConversationMessageAdd' id=$conversationID}{/link}" title="{lang}wcf.conversation.message.add{/lang}" class="btn btn-primary btn-3d jsQuickReply"><span class="icon icon16 icon-arrow-left"></span> <span>{lang}wcf.conversation.message.button.add{/lang}</span></a></li>{/if}
  ...
  <div class="contentNavigation marginBottom-30">
  	{@$pagesLinks}
  	...
  	{hascontent}
  		<nav>
  			<ul>
  				{content}
  					{if !$conversation->isClosed}<li><a href="{link controller='ConversationMessageAdd' id=$conversationID}{/link}" title="{lang}wcf.conversation.message.add{/lang}" class="btn btn-primary btn-3d jsQuickReply"><span class="icon icon16 icon-plus"></span> <span>{lang}wcf.conversation.message.button.add{/lang}</span></a></li>{/if}
