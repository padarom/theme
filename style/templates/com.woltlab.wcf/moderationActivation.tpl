...
{capture assign="headerTitle"}
	{lang}wcf.moderation.activation{/lang}: {$queue->getTitle()}
{/capture}

{include file='header' title=$headerTitle light=true}

<div class="container">
	<header class="boxHeadline">
		{if $queue->lastChangeTime}
			<dl class="plain inlineDataList">
				<dt style="min-height:0;">{lang}wcf.moderation.lastChangeTime{/lang}</dt>
				<dd>{@$queue->lastChangeTime|time}</dd>
			</dl>
		{/if}

		<dl class="plain inlineDataList" id="moderationAssignedUserContainer">
			<dt style="min-height:0;">{lang}wcf.moderation.assignedUser{/lang}</dt>
			<dd>
				<span>
		...
		<dl class="plain inlineDataList" id="moderationStatusContainer">
			<dt style="min-height:0;">{lang}wcf.moderation.status{/lang}</dt>
