{if $__wcf->user->userID}
					...
					<li>
						<div class="box64">
							...
							<div class="containerHeadline">
								...
								{if MODULE_USER_RANK}
									<p style="line-height:12px;font-size:11px;padding-bottom:25px;padding-top:3px;">
										{if $__wcf->getUserProfileHandler()->getUserTitle()}
											<span class="{if $__wcf->getUserProfileHandler()->getRank() && $__wcf->getUserProfileHandler()->getRank()->cssClassName} {@$__wcf->getUserProfileHandler()->getRank()->cssClassName}{/if}">{@$__wcf->getUserProfileHandler()->getUserTitle()}</span>
										{/if}
	...
	{if $__hideUserMenu|empty}
		<li id="userNotifications" class="hidden-xs" data-count="{#$__wcf->getUserNotificationHandler()->getNotificationCount()}">
			<a href="{link controller='NotificationList'}{/link}"><span class="icon icon16 icon-bell-alt {if $__wcf->getUserNotificationHandler()->getNotificationCount()}active{/if}"></span>{if $__wcf->getUserNotificationHandler()->getNotificationCount()} <span class="badge circular">{#$__wcf->getUserNotificationHandler()->getNotificationCount()}</span>{/if}</a>
...
{if $__hideUserMenu|empty}
	{if $__wcf->user->userID && $__wcf->session->getPermission('mod.general.canUseModeration')}
		<li id="outstandingModeration" class="hidden-xs" data-count="{#$__wcf->getModerationQueueManager()->getOutstandingModerationCount()}">
			<a href="{link controller='ModerationList'}{/link}">
				<span class="icon icon16 icon-warning-sign {if $__wcf->getModerationQueueManager()->getUnreadModerationCount()}active{/if}"></span>
				{if $__wcf->getModerationQueueManager()->getUnreadModerationCount()}<span class="badge circular">{#$__wcf->getModerationQueueManager()->getUnreadModerationCount()}</span>{/if}
			</a>
			{if !OFFLINE || $__wcf->session->getPermission('admin.general.canViewPageDuringOfflineMode')}
				<script data-relocate="true">
					//<![CDATA[
