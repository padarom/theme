...
{include file='header' sidebarOrientation='left' title='wcf.user.notification.notifications'|language light=true paddingBottom=30}
...
{if $notifications[notifications]}
	...
	{foreach from=$notifications[notifications] item=$notification}
		{if $notification[event]->getPeriod() != $lastPeriod}
			...
			<header class="boxHeadline boxSubHeadline container">
				<h2>{$lastPeriod}</h2>
			</header>
