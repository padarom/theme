...
{include file='header' sidebarOrientation='left' title=$user->username light=true}
    <div class="container marginBottom-30">
        <div class="row">
            <div class="col-md-4">
                <div class="framed marginBottom-30">
                    {@$user->getAvatar()->getImageTag(300)}
                </div>
            </div>
            <div class="col-md-8">

                <h2 class="mb-none big">{$user->username}</h2>
                <p style="line-height:20px;">{@$user->getUserTitle()}</p>
				...
				{event name='beforeUserInfoBlock'}
				
        {event name='afterUserInfoSection'}

				{event name='afterUserInfoBlock'}

            </div>
        </div>
		
        {event name='beforeUserInfoSection'}

		<div class="heading heading-border heading-middle-border">
			<h3 class="big"><span class="inverted inverted-quarternary">User</span> Information</h3>
		</div>
        ...
        <div class="tabs tabs-primary">
            <ul class="nav nav-tabs">
            	{foreach from=$__wcf->getUserProfileMenu()->getMenuItems() item=menuItem}
                    {if $menuItem->getContentManager()->isVisible($userID)}
                        <li class="{if $menuItem === $__wcf->getUserProfileMenu()->getActiveMenuItem()}active{/if}" data-active="{$__wcf->getUserProfileMenu()->getActiveMenuItem()->getIdentifier()}">
                            <a href="{$__wcf->getAnchor($menuItem->getIdentifier())}" data-toggle="tab">
                                {lang}wcf.user.profile.menu.{@$menuItem->menuItem}{/lang}
                            </a>
                        </li>
                    {/if}
                {/foreach}
            ...
            <div class="tab-content">
                {foreach from=$__wcf->getUserProfileMenu()->getMenuItems() item=menuItem}
                    {if $menuItem->getContentManager()->isVisible($userID)}
                        <div id="{$menuItem->getIdentifier()}" class="tab-pane {if $menuItem === $__wcf->getUserProfileMenu()->getActiveMenuItem()}active{/if}">
                            {@$menuItem->getContentManager()->getContent($user->userID)}
                        </div>
                    {/if}
                {/foreach}
            </div>
